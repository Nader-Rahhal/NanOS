...
inline void set_descriptor(IDT* idt, uint8_t vector, void* isr, uint8_t flags){
...
inline void init() {
    static detail::IDT idt;
    static detail::IDTR idtr;

    idtr.base  = (uint64_t)&idt;
    idtr.limit = (uint16_t)sizeof(detail::IDT) - 1;

    for (int vector = 0; vector < 256; vector++) {
        detail::set_descriptor(&idt, vector, isr_stub_table[vector], 0x8E);
    }
